<template>
  <div class="player" v-show="musicUrl">
...
export default {
  ...
  methods: {
    _getSongvKey () {
      getSongvKey(this.currentSong.songmid).then((res) => {
        if (res.code === ERR_OK) {
          this.musicUrl = `${this.currentSong.url}?vkey=${res.data.items[0].vkey}&guid=4851870848&uin=0&fromtag=66`
          this.$nextTick(() => {
